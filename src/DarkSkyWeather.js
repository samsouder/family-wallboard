...
class DarkSkyWeather extends PureComponent {
  state: {
    apiData?: {}
  };
  refreshTimer: number;
  ...
  render() {
    ...
    const currentPrecipIcon =
      this.state.apiData.currently.apparentTemperature <= 32 ? "wi-snowflake-cold" : "wi-raindrop";

    return (
      <div className="DarkSkyWeather">
        <div className="currentTemperature">
          {Math.round(this.state.apiData.currently.temperature)}°{" "}
          <i className={"wi wi-forecast-io-" + this.state.apiData.currently.icon} />
        </div>
        ...
        <div className="sunrise">
          {this.state.apiData.daily.data[0].sunriseDateTime.format("h:mm")} <i className="wi wi-sunrise" />
        </div>
        <div className="sunset">
          {this.state.apiData.daily.data[0].sunsetDateTime.format("h:mm")} <i className="wi wi-sunset" />
        </div>
